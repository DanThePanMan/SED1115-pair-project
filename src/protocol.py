...
def _create_general_frame(data: bytes) -> bytes:
    """
    Create a general frame given a body.
    ...
    # write the start header byte
    output.append(_frame_header)

    # escape the header and body
    escaped_body = escape_content(data)

    # calculate and escape checksum
    checksum = calculate_checksum(escaped_body).to_bytes(1, 'big')
    escaped_checksum = escape_content(checksum)

    output.extend(escaped_body)
    output.extend(escaped_checksum)
